 ...
 *
 * Why: the app is served by Work Zone out of the HTML5 Application Repository,
 * at /<site-id>.<cloud-service>.<app-id>-<version>/~<cachebuster>~/... . The
 * ~token~ makes the inner resource paths unique per deploy, but the outer
 * segment only moves when the manifest version moves. Redeploying the same
 * version leaves index.html on a path the browser has already cached, which is
 * why testers were seeing stale builds until they cleared their cache. A unique
 * version per build changes that outer segment, so there is nothing left to
 * serve from cache.
 *
